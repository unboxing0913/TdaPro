...
로그인성공후 처리를 담당하는 핸들러 제거
---> 만들어두었지만 따로 사용할일이 없고 어노테이션을 통해 각 페이지별로 권한설정할것이기때문

@PreAuthorize("isAuthenticated()") 어노테이션을 통한 로그인인이 성공한 사용자만 사용할수있도록 권한설정
---> ReviewController의 등록 

게시물 등록시 로그인한 사용자 아이디의 출력
---> 로그인한 사용자의 아이디가 writer항목에 사용될수있도록 register.jsp 수정

비로그인 등록시 로그인창으로 넘어가지않고 500 에러가 발생
---> 문제는 servlet.context.xml안에 있는  <context:component-scan base-package="org.unboxing.exception" /> 에서문제가 발생 주석처리후 진행

게시물 조회와 로그인처리
--->  로그인한 사용자가 게시물 작성자일 경우에 수정/삭제 작업을 할수있는 기능이 활성화될수있게 get.jsp에서 처리

조회 화면에서 댓글추가 버튼 
---> 로그인한 사용자만이 댓글 추가버튼이 활성화 되도록 <sec:authrize>를 이용해 처리

게시물의 수정/삭제부분을 URL을 조작해서 쉽게 접근이 가능하기때문에 알맞게 처리 (modify.jsp)
---> POST방식으로 처리되는부분에서 CSRF 토근과 시큐리티를 적용
---> 조회와 마찬가지로 현재 로그인사용자가 게시물작성자일 경우에만 수정/삭제버튼이 활성화되게 처리

ReviewController에서의 제어
---> 메서드를 실행하기전 로그인한 사용자와 현재 파라미터로 전달되는 작성자가 일치하는지 체크
---> 삭제의 경우는 bno만을 파라미터로 받고있었지만 작성자를 의미하는 writer 를 추가해 @PreAuthorize로 검사 (modifty는 ReviewVO를 파라미터로 가지고있음)

게시물 등록시 첨부파일의 처리
---> 게시물 등록이 POST 방식으로 전송되고있기때문에 JavaSprint 수정
---> 시큐리티 적용시 POST , PUT , PATCH , DELETE 와 같은 방식으로 데이터를 전송하는경우 'X-CSRF-TOKEN'과 같은 헤더정보를 추가해 CSRF토큰값을 전달
---> 첨부파일 등록 , 삭제 모두 POST이기 때문에 전송방식에서 토큰처리를 해준다.
---> 게시물의 수정화면에서도 첨부파일은 추가/삭제가 가능하기때문에 코드를 수정해준다. (modify.jsp)

ReviewController의 Upload 관련된 매핑 메소드안에 로그인한 사용자만 사용할수있도록 제한


댓글의 경우도 Ajax로 이루어져있기때문에 화면과 서버쪽 변경을 해줌
---> 댓글 등록의 경우 로그인한 사용자만 댓글을 추가할수있도록 수정
---> 댓글 수정이나 삭제의 경우 작성자의 아이디와 로그인한 아이디가 같을 경우 수정/삭제 할수있도록 수정
---> 댓글등록의 경우 CSRF 토큰을 같이 전송하도록 해준다
---> 수정/삭제 의 경우에는 댓글번호만으로 처리하는게아닌 댓글 작성자를 같이 전송하도록 처리

삭제의 경우 로그인한 유저가 작성한 댓글만 삭제가능하도록 처리 

댓글의 수정과 삭제 처리는 댓글작성자 항목을 같이전송하도록 수정
---> 댓글번호 (rno)만 전송했던것을 댓글 작성자를 같이 전송하도록 수정
---> resources 폴더안의 js/review_reply.js 에서 rno과 replyer를 같이전송하도록 수정

댓글수정
---> 기존의 댓글 내용 + 댓글의 작성자가 같이 전송될수있도록 수정

로그아웃처리 
---> 시큐리티 태그를 이용해 로그인한상태에서는 header부분의 login 버튼이 logout으로 보이게 수정


회원가입처리
---> 회원가입 mapper 와 회원의 권한 mapper를 service에서 처리 권한은 기본 ROLE_USER로 설정
---> 권한은 추후 회원테이블을 만들어 관리자권한으로 등급을 바꿀수있도록 계획

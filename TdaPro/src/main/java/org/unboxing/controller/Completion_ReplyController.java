...
import org.unboxing.service.Completion_ReplyService;

import lombok.AllArgsConstructor;
import lombok.extern.log4j.Log4j;

@RequestMapping("/completion/replies/")
@RestController  // 컨트롤러가 REST방식으로 처리하기위한것임을 명시함
@Log4j
@AllArgsConstructor
public class Completion_ReplyController {

	
	private Completion_ReplyService service;
...
// @PathVariable("") : URL 경로에 있는 값을 파라미터로 추출하려고 할때 사용 
	...
	public ResponseEntity<String> create(@RequestBody Completion_ReplyVO vo){
		
		log.info("Completion_ReplyVO : " + vo );
	...
	public ResponseEntity<List<Completion_ReplyVO>> getList(
	...
	*/
	...
	@GetMapping(value="/pages/{bno}/{page}",
			produces = {MediaType.APPLICATION_XML_VALUE , MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<Completion_ReplyPageDTO> getList(@PathVariable("page") int page , @PathVariable("bno") Long bno){
	...
	@GetMapping(value="/{rno}",
			produces = {MediaType.APPLICATION_XML_VALUE , MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<Completion_ReplyVO> get(@PathVariable("rno") Long rno){
	...
	public ResponseEntity<String> remove(@PathVariable("rno") Long rno,@RequestBody Completion_ReplyVO vo){
	...
	public ResponseEntity<String> modify(
			@RequestBody Completion_ReplyVO vo,

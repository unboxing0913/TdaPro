...
public interface ReviewAttachMapper {
	...
	public void insert(ReviewAttachVO vo); //첨부파일 등록
	
	public void delete(String uuid); //첨부파일 삭제
	
	public List<ReviewAttachVO> findByBno(Long bno); // 등록과 삭제를 하기위한 특정 게시물 번호 찾기
	
	public void deleteAll(Long bno); //게시물 삭제시 첨부파일 전체삭제
	
	public List<ReviewAttachVO> getOldFiles(); // task 스케쥴러를사용해 잘못업로드된 파일을 제거하기위함
}
